...
QString AMaterialComposition::fillNaturalAbundances(const QMap<QString, double> & map, QVector<AChemicalElement> & tmpElements, bool KeepIsotopComposition)
{
    ...
    const AIsotopeAbundanceHandler & IsoAbHandler = AIsotopeAbundanceHandler::getInstance();
    for (const QString & el : qAsConst(ListOfElements))
    {
    ...
    for (const QString & ElementName : qAsConst(ListOfElements))
    {
        ...
        {
            for (const AChemicalElement & el : qAsConst(ElementComposition))
...
QString AMaterialComposition::setCompositionString(const QString composition, bool KeepIsotopComposition)
{
    ...
    for (const QPair<QString,double> & pair : qAsConst(Records))
    {
...
QString AMaterialComposition::setCompositionByWeightString(const QString composition)
{
    ...
    double sumMass = 0;
    for (const AChemicalElement & el : qAsConst(tmpElements))
...
{
    jstools::parseJson(json, "ElementCompositionString", ElementCompositionString);
    QJsonArray ar;
    jstools::parseJson(json, "ElementComposition", ar);
