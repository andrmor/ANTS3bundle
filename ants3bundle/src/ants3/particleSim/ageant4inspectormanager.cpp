...
bool AGeant4InspectorManager::inspectMaterial(const QString & matName, bool nCrystalMaterial, AG4MaterialRecord & reply)
{
    ...
    else
    {
        ...
        else
        {
            ...
            jstools::parseJson(js, "Name",            reply.Name);
...
bool AGeant4InspectorManager::requestVersion(QString & version)
{
    bAborted = false;
    ErrorString.clear();
    ...
    A3WorkDistrConfig Request;
    Request.NumEvents = numEvents;
    bool ok = configureForRequestGeantVersion(RunPlan, Request);
    if (!ok) return false;
...
bool AGeant4InspectorManager::configureForRequestGeantVersion(std::vector<AFarmNodeRecord> &RunPlan, A3WorkDistrConfig &Request)
{
    Request.Command = "g4inspector";
