...
{
    if ( !jstools::parseJson(json, "SigmaAlpha", sigma_alpha)) return false;
    if ( !jstools::parseJson(json, "SigmaH", sigma_h)) return false;
    if ( !jstools::parseJson(json, "Albedo", albedo)) return false;

    int ival;
    if ( !jstools::parseJson(json, "HDmodel", ival)) return false;
    if (ival<0 || ival>2) return false;
    HeightDistribution = static_cast<HeightDistrEnum>(ival);

    if ( !jstools::parseJson(json, "SDmodel", ival)) return false;
    if (ival<0 || ival>2) return false;
