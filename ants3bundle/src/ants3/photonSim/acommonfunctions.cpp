...
double GetRandomFromHist(TH1 *hist)
{
    ...
    double r1 = ARandomHub::getInstance().uniform();
...
int GetRandomBinFromHist(TH1 *hist)
{
    int nbinsx = hist->GetNbinsX();
    double * fIntegral = hist->GetIntegral();
    double integral = fIntegral[nbinsx];
...
bool NormalizeVector(double *arr)
{
    ...
    if (Norm < 1e-20)
      {
        // qDebug()<<"fail norm";
        guitools::message("Error normalizing to unit vector!");
        return false;
      }
...
bool ExtractNumbersFromQString(QString input, QList<int> *ToAdd)
{
  ...
  if (fields.size() == 0 )
    {
      //guitools::message("Nothing to add!");
      return false;
    }
