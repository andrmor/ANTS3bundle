   ...
   QFrame(parent), SensorSignals(sensorSignals)
{
    ...
    gvScale = new ASensorGView(this);
    gvScale->setScene(scaleScene);

    labMax     = new QLabel("--");
    labHighMid = new QLabel("--");
    labMid     = new QLabel("--");
    labLowMid  = new QLabel("--");

    pbResetView = new QPushButton("Reset view");
    QObject::connect(pbResetView, &QPushButton::clicked, this, &ASensorViewer::onResetViewportClicked);

    sbDecimals = new QSpinBox();
    sbDecimals->setValue(2);

    // laying out gui
    QHBoxLayout * layMain = new QHBoxLayout(this);
    layMain->addWidget(gvOut);
        QVBoxLayout * layRight = new QVBoxLayout();
            QHBoxLayout * layTop = new QHBoxLayout();
            layTop->addWidget(gvScale);
                QVBoxLayout * layLabels = new QVBoxLayout();
                layLabels->addWidget(labMax);
                layLabels->addWidget(labHighMid);
                layLabels->addWidget(labMid);
                layLabels->addWidget(labLowMid);
                layLabels->addWidget(new QLabel("0"));
            layTop->addLayout(layLabels);
        layRight->addLayout(layTop);
        layRight->addWidget(pbResetView);
            QHBoxLayout * layH1 = new QHBoxLayout();
            layH1->addWidget(new QLabel("Decimals:"));
            layH1->addWidget(sbDecimals);
        layRight->addLayout(layH1);
        layRight->addSpacerItem(new QSpacerItem(20,20));
...
void ASensorViewer::clearGrItems()
{
...
void ASensorViewer::updateSignalLabels(float MaxSignal)
{
    QString str;
    ...
    if (lowmid>0)
    {
        ...
        labLowMid->setText(str);
    }
    else labLowMid->setText("");
    ...
    if (highmid > 0 && highmid !=mid)
    {
        ...
        labHighMid->setText(str);
    }
    else labHighMid->setText("");
}

void ASensorViewer::resetViewport()
{
...
void ASensorViewer::addSensorItems(float MaxSignal)
{
    const ASensorHub & SensorHub = ASensorHub::getConstInstance();
    const int numSensors = SensorHub.countSensors();
...
void ASensorViewer::addTextItems(float MaxSignal)
{
    const ASensorHub & SensorHub = ASensorHub::getConstInstance();
    const int numSensors = SensorHub.countSensors();

    const int prec = sbDecimals->value();
    for (int iSens = 0; iSens < numSensors; iSens++)
    {
        AGeoObject * obj = SensorHub.SensorData[iSens].GeoObj;
        if (!obj->Shape)
