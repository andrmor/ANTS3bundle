    ...
    AInterfaceRuleWidget(parent), Rule(rule)
{
    QVBoxLayout * vl = new QVBoxLayout(this);
        QHBoxLayout * l = new QHBoxLayout();
            ...
            pb->setToolTip("Load a surface LUT file generated by ray tracing over a 3D surface topography:\nsee generateSurfaceLut() method of the 'rules' script unit");
            QObject::connect(pb, &QPushButton::clicked, this, &ALutInterfaceWidget::loadLut);
        l->addWidget(pb);
            pbShowProbabilities = new QPushButton("R/T vs angle");
            pbShowProbabilities->setToolTip("Draw the reflection and transmission probabilities vs the incidence angle");
            QObject::connect(pbShowProbabilities, &QPushButton::clicked, this, &ALutInterfaceWidget::showProbabilities);
        l->addWidget(pbShowProbabilities);
    vl->addLayout(l);
        labInfo = new QLabel("No LUT loaded");
    vl->addWidget(labInfo);
        labWarning = new QLabel("");
        labWarning->setStyleSheet("QLabel {color: red;}");
    vl->addWidget(labWarning);
        l = new QHBoxLayout();
            QLabel * lab = new QLabel("Angular distribution for incidence angle:");
        l->addWidget(lab);
            leAngle = new QLineEdit("45");
            QDoubleValidator * val = new QDoubleValidator(this);
            val->setNotation(QDoubleValidator::StandardNotation);
            val->setBottom(0);
            val->setDecimals(6);
        ...
        l->addWidget(leAngle);
            lab = new QLabel("deg");
        l->addWidget(lab);
            cobBranch = new QComboBox();
            cobBranch->addItem("Reflected"); cobBranch->addItem("Transmitted");
        l->addWidget(cobBranch);
            pbShowDistribution = new QPushButton("Show");
            QObject::connect(pbShowDistribution, &QPushButton::clicked, this, &ALutInterfaceWidget::showDistribution);
        l->addWidget(pbShowDistribution);
    vl->addLayout(l);

    updateGui();
}

void ALutInterfaceWidget::loadLut()
{
    QString fileName = guitools::dialogLoadFile(Parent, "Load surface LUT", "Surface LUT files (*.lut *.json);;All files (*)");
    if (fileName.isEmpty()) return;

    QString err = Rule->loadLUT(fileName);
    if (!err.isEmpty()) guitools::message(err, Parent);

    updateGui();
}

void ALutInterfaceWidget::updateGui()
{
    const bool loaded = Rule->Data.isLoaded();
    if (loaded)
    {
        const ALutSurfaceData & data = Rule->Data;
        labInfo->setText(QString("n %1 -> %2 at %3 nm  |  %4 incidence bins, out %5 theta x %6 phi  |  mean bounces: %7")
                         .arg(data.n1).arg(data.n2).arg(data.Wavelength)
                         .arg(data.ThetaIncBins).arg(data.ThetaOutBins).arg(data.PhiOutBins)
                         .arg(data.MeanBounces, 0, 'g', 3));
        labWarning->setText(Rule->getMaterialConsistencyWarning().trimmed());
    }
    else
    {
        labInfo->setText("No LUT loaded");
        labWarning->clear();
    }
    pbShowProbabilities->setEnabled(loaded);
    pbShowDistribution->setEnabled(loaded);
}

void ALutInterfaceWidget::showProbabilities()
{
    const ALutSurfaceData & data = Rule->Data;
    if (!data.isLoaded())
    {
        guitools::message("LUT is not loaded", Parent);
        return;
    }

    const double binWidth = 90.0 / data.ThetaIncBins;
    std::vector<double> angles(data.ThetaIncBins), probR(data.ThetaIncBins), probT(data.ThetaIncBins);
    for (int i = 0; i < data.ThetaIncBins; i++)
    {
        angles[i] = (i + 0.5) * binWidth;
        probR[i]  = (data.Launched[i] > 0 ? data.ReflectedCounts[i]   / (double)data.Launched[i] : 0);
        probT[i]  = (data.Launched[i] > 0 ? data.TransmittedCounts[i] / (double)data.Launched[i] : 0);
    }

    TGraph * gR = AGraphBuilder::graph(angles, probR);
    AGraphBuilder::configure(gR, "Reflection", "Incidence angle, deg", "Probability", 2, 20, 1, 2);
    gR->SetMinimum(0);
    gR->SetMaximum(1.05);
    emit requestDraw(gR, "ALP", true, false);

    TGraph * gT = AGraphBuilder::graph(angles, probT);
    AGraphBuilder::configure(gT, "Transmission", "Incidence angle, deg", "Probability", 4, 21, 1, 4);
    emit requestDraw(gT, "LPsame", true, true);

    emit requestDrawLegend(0.7, 0.8, 0.95, 0.95, "");
}

void ALutInterfaceWidget::showDistribution()
{
    const ALutSurfaceData & data = Rule->Data;
    if (!data.isLoaded())
    {
    ...
    const double angle = leAngle->text().toDouble();
    if (angle < 0 || angle >= 90.0)
    {
        guitools::message("Incidence angle should be in the range [0, 90) degrees", Parent);
        return;
    }
    int iBin = (int)(angle / 90.0 * data.ThetaIncBins);
    if (iBin >= data.ThetaIncBins) iBin = data.ThetaIncBins - 1;

    const bool reflected = (cobBranch->currentIndex() == 0);
    const std::vector<int> & hist = (reflected ? data.ReflectedHist[iBin] : data.TransmittedHist[iBin]);

    const double binWidth = 90.0 / data.ThetaIncBins;
    TH2D * h2 = new TH2D("hLutAngDist", QString("%1 photons for incidence angles [%2, %3) deg")
                             .arg(reflected ? "Reflected" : "Transmitted")
                             .arg(iBin * binWidth).arg((iBin + 1) * binWidth).toLatin1().data(),
                         data.PhiOutBins, 0, 360.0, data.ThetaOutBins, 0, 90.0);
    h2->GetXaxis()->SetTitle("Azimuth from the incidence plane, deg");
    h2->GetYaxis()->SetTitle("Polar angle from the normal, deg");
    for (int iTheta = 0; iTheta < data.ThetaOutBins; iTheta++)
        for (int iPhi = 0; iPhi < data.PhiOutBins; iPhi++)
            h2->SetBinContent(iPhi + 1, iTheta + 1, hist[iTheta * data.PhiOutBins + iPhi]);
