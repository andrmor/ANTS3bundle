...
{
    bool bConfirm = guitools::confirm("Remove all entries?", this);
    if (!bConfirm) return;
...
void ALegendDialog::on_twTree_itemDoubleClicked(QTreeWidgetItem *item, int)
{
    ...
    {
        guitools::message("Bad index - the tree widget is corrupted", this);
        return;
    }
    ...
    {
        guitools::message("Unsupported object type", this);
        return;
    }
...
{
    guitools::inputInteger("Number of columns for entries:", CurrentModel.NumColumns, 1, 10, this);
    updateLegend();
}

void ALegendDialog::onHelpTriggered()
{
    guitools::message("Double-click on a draw object to add entry\n"
